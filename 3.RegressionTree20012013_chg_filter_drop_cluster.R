...
for (j in 1:length(LDspec)) {
	...
	dat1 <- cbind("pred"=dat[,5],dat[,c(2,11:(ncol(dat)-4))])
	...
	if (class(x1) == "NULL") {x1<-try(brt1 <- gbm.step(dat1, gbm.y = 1, gbm.x = c(2:ncol(dat1)), family = "gaussian", tree.complexity = 3, learning.rate = 0.0001, bag.fraction = 0.5))}
...
for (j in 21:length(LDspec)) {
	...
	for (i in levels(dat$region)) {
		dat1 <- cbind("pred"=dat[,5],dat[,c(2,11:(ncol(dat)-4))],"region"=dat[,ncol(dat)])
		...
		if (class(x1) == "NULL") {x1 <- try(brt1 <- gbm.step(dat1, gbm.y = 1, gbm.x = c(2:(ncol(dat1)-1)), family = "gaussian", tree.complexity = 3, learning.rate = 0.0001, bag.fraction = 0.5))}
		if (class(x1) != "try-error") {
		  if(class(x1) != "NULL") {
...
for (j in 1:length(LDspec)) {
  ...
  x1<- try(brt1 <- gbm.step(dat1, gbm.y = 1, gbm.x = c(2:ncol(dat1)), family = "gaussian", tree.complexity = 3, learning.rate = 0.001, bag.fraction = 0.5))
  if (class(x1) == "NULL") {x1<- try(brt1 <- gbm.step(dat1, gbm.y = 1, gbm.x = c(2:ncol(dat1)), family = "gaussian", tree.complexity = 3, learning.rate = 0.001, bag.fraction = 0.5))}
  if (class(x1) != "try-error") {
    if(class(x1) != "NULL") {
...
for (j in 1:length(LDspec)) {
	...
	# tc$region <- as.factor(tc$region)
	for (i in levels(dat$region)) {
		dat1 <- cbind("chg"=dat[,9],dat[,c(2,11:(ncol(dat)-4))],"region"=dat[,ncol(dat)])
		dat1 <- dat1[dat1$region == i,]
		if(nrow(dat1>0)){
		x1 <- try(brt1 <- gbm.step(dat1, gbm.y = 1, gbm.x = c(2:(ncol(dat1)-1)), family = "gaussian", tree.complexity = 3, learning.rate = 0.001, bag.fraction = 0.5))
		  if (class(x1) == "NULL") {x1 <- try(brt1 <- gbm.step(dat1, gbm.y = 1, gbm.x = c(2:(ncol(dat1)-1)), family = "gaussian", tree.complexity = 3, learning.rate = 0.0001, bag.fraction = 0.5))}
...
for (j in 1:length(LDspec)) {
  ...
  dat$anom <- (dat$pred - dat$mean)/(dat$sd)
  dat <- dat[dat$mean > 0.001,]
  dat1 <- cbind("tanom"=dat[,10],dat[,c(2,11:(ncol(dat)-3))])
  x1<- try(brt1 <- gbm.step(dat1, gbm.y = 1, gbm.x = c(2:ncol(dat1)), family = "gaussian", tree.complexity = 3, learning.rate = 0.001, bag.fraction = 0.5))
  if (class(x1) == "NULL") {x1<- try(brt1 <- gbm.step(dat1, gbm.y = 1, gbm.x = c(2:ncol(dat1)), family = "gaussian", tree.complexity = 3, learning.rate = 0.001, bag.fraction = 0.5))}
  if (class(x1) != "try-error") {
    if(class(x1) != "NULL") {
      save(brt1,file=paste("G:/Boreal/InterannualVariability/",LDspec[j],"brt_2001-2013_tanom_update.RData",sep=""))
      varimp <- as.data.frame(brt1$contributions[1:100,])
      write.csv(varimp,file=paste("G:/Boreal/InterannualVariability/",LDspec[j],"brt_2001-2013varimp_tanom_update.csv",sep=""))
      cvstats <- as.data.frame(brt1$cv.statistics[c(1,3)])
      names(cvstats) <- c("deviance.cv","correlation.cv")
      cvstats$deviance.null <- brt1$self.statistics$mean.null
      cvstats$pseudo.R2 <- (brt1$self.statistics$mean.null-brt1$self.statistics$mean.resid)/brt1$self.statistics$mean.null
      cvstats$correlation <- brt1$self.statistics$correlation
      cvstats$pseudo.R2.cv <- (cvstats$deviance.null-cvstats$deviance.cv)/cvstats$deviance.null
      write.csv(cvstats,file=paste("G:/Boreal/InterannualVariability/",LDspec[j],"brt_2001-2013cvstats_tanom_update.csv",sep=""))
    }
  }
}	
...
for (j in 1:length(LDspec)) {
	...
	tc$region <- as.factor(tc$region)
	for (i in levels(dat$region)) {
		dat1 <- cbind("tanom"=dat[,10],dat[,c(2,11:(ncol(dat)-4))],"region"=dat[,ncol(dat)])
		dat1 <- dat1[dat1$region == i,]
		if(nrow(dat1)>0){	
		x1 <- try(brt1 <- gbm.step(dat1, gbm.y = 1, gbm.x = c(2:(ncol(dat1)-1)), family = "gaussian", tree.complexity = 3, learning.rate = 0.001, bag.fraction = 0.5))
		if (class(x1) == "NULL") {x1 <- try(brt1 <- gbm.step(dat1, gbm.y = 1, gbm.x = c(2:(ncol(dat1)-1)), family = "gaussian", tree.complexity = 3, learning.rate = 0.0001, bag.fraction = 0.5))}
...
for (j in 1:length(LDspec)) {
  ...
  dat$anom <- (dat$pred - dat$mean)/(dat$sd)
  dat <- dat[dat$mean > 0.001,]
  dat1 <- cbind("anom"=dat[,ncol(dat)-1],dat[,c(2,11:(ncol(dat)-3))])
  x1<- try(brt1 <- gbm.step(dat1, gbm.y = 1, gbm.x = c(2:ncol(dat1)), family = "gaussian", tree.complexity = 3, learning.rate = 0.001, bag.fraction = 0.5))
  if (class(x1) == "NULL") {x1<- try(brt1 <- gbm.step(dat1, gbm.y = 1, gbm.x = c(2:ncol(dat1)), family = "gaussian", tree.complexity = 3, learning.rate = 0.001, bag.fraction = 0.5))}
  if (class(x1) != "try-error") {
    if(class(x1) != "NULL") {
...
for (j in 1:length(LDspec)) {
  ...
  tc$region <- as.factor(tc$region)
  for (i in levels(dat$region)) {
    dat1 <- cbind("anom"=dat[,ncol(dat)-1],dat[,c(2,11:(ncol(dat)-4))],"region"=dat[,ncol(dat)])
    dat1 <- dat1[dat1$region == i,]
    if(nrow(dat1>0)){
    x1 <- try(brt1 <- gbm.step(dat1, gbm.y = 1, gbm.x = c(2:(ncol(dat1)-1)), family = "gaussian", tree.complexity = 3, learning.rate = 0.001, bag.fraction = 0.5))
    if (class(x1) == "NULL") {x1 <- try(brt1 <- gbm.step(dat1, gbm.y = 1, gbm.x = c(2:(ncol(dat1)-1)), family = "gaussian", tree.complexity = 3, learning.rate = 0.0001, bag.fraction = 0.5))}
...
for (i in 1:length(cv)) {
  ...
  combo$metric[i] <- substr(cv[i],26,31)
...
length(unique(combo$species)) #n=44 species
...
cv <- !grep(cv, pattern="eco",value=TRUE)vi <- grep(vi, pattern="chg", value=TRUE)
cvstats <- read.csv(cv[1])
cvstats$spec <- substr(cv[1],1,5)
...
cv <- grep(cv, pattern="chg_cluster", value=TRUE) #95 region / species
vi <- grep(vi, pattern="chg_cluster", value=TRUE)
cvstats <- read.csv(cv[1])
...
s <- aggregate(varimp$R2, by=list(varimp$species),FUN="max") #44 species
