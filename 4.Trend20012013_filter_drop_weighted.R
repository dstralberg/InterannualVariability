...
clust <- raster("G:/Boreal/InterannualVariability/boreal_ecoregions_drop.asc", RAT=TRUE)
...
for (j in 1:length(LDspec)) {
	spectrend <- trend[trend$SPECIES == as.character(LDspec[j]),]
		rc <- ratify(clust)
		ratc <- levels(rc)[[1]]
		ratc <- merge(ratc,spectrend[,c(2,4)],by.x="ID",by.y="ecodrop")
		ratc$beta <- round(ratc$beta, 3)
		#levels(rc) <- ratc$beta
		#rc1 <- deratify(rc,att='beta')
		plot1 <- levelplot(rc, att='beta', main = paste(LDspec[j], sep=" "), margin=FALSE) + layer(sp.polygons(prov))
