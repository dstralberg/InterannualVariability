...
for (j in 1:length(LDspec)) {
	...
	dat1 <- na.omit(dat1)
	try(brt.chg <- gbm.step(dat1, gbm.y = ncol(dat1), gbm.x = c(3:(ncol(dat1)-1)), family = "gaussian", tree.complexity = 3, learning.rate = 0.001, bag.fraction = 0.5))
	...
	try(save(brt.chg,file=paste("L:/Boreal/InterannualVariability/",LDspec[j],"brt_2001-2013_chg_eco.RData",sep="")))	
	varimp <- as.data.frame(brt.chg$contributions)
	write.csv(varimp,file=paste("L:/Boreal/InterannualVariability/",LDspec[j],"brt_2001-2013varimp_chg_eco.csv",sep=""))
	cvstats <- t(as.data.frame(brt.chg$cv.statistics))
	write.csv(cvstats,file=paste("L:/Boreal/InterannualVariability/",LDspec[j],"brt_2001-2013cv_chg_eco.csv",sep=""))
}	
...
cl <- list.files("L:/Boreal/InterannualVariability/", pattern="eco.csv")
...
write.csv(combo, file="L:/Boreal/InterannualVariability/_BRTResults_eco.csv")
...
write.csv(cvstats, file="L:/Boreal/InterannualVariability/_cvstats_chg_eco.csv", row.names=FALSE)
...
write.csv(varimp, file="L:/Boreal/InterannualVariability/_varimp_chg_eco.csv", row.names=FALSE)

topvars <- aggregate(varimp[,c(2,9)], by=list("var"=varimp$var), FUN="sum")
write.csv(topvars, file="L:/Boreal/InterannualVariability/_topvars_chg_eco.csv", row.names=FALSE)
...
write.csv(s, file="L:/Boreal/InterannualVariability/_speciesR2_eco.csv"

varimpsum <- aggregate(varimp[,c(2,9)],by=list("type"=varimp$type), FUN="sum")	
write.csv(varimpsum, file="L:/Boreal/InterannualVariability/_varimpsum_chg_eco.csv", row.names=FALSE)

varimpsum2 <- aggregate(varimp[,c(2,9)],by=list("type2"=varimp$level3), FUN="sum")	
write.csv(varimpsum2, file="L:/Boreal/InterannualVariability/_varimpsum_chg_eco2.csv", row.names=FALSE)

varimpmean <- aggregate(varimp[,c(2,9)],by=list(varimp$type), FUN="mean")	
write.csv(varimpmean, file="L:/Boreal/InterannualVariability/_varimpmean_chg_eco.csv", row.names=FALSE)

varimpmean2 <- aggregate(varimp[,c(2,9)],by=list(varimp$level3), FUN="mean")	
write.csv(varimpmean2, file="L:/Boreal/InterannualVariability/_varimpmean_chg_eco2.csv", row.names=FALSE)
