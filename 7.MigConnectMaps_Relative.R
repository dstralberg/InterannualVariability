...
for (i in 1:length(LDspec)) {
  pdf(paste("G:/Boreal/InterannualVariability/",LDspec[i],"_migconnmaps_update_relative.pdf",sep=""))
	...
	v <- v[v$wint1 != 52,]
	if(nrow(v)>0) {
	range <- rgdal::readOGR(paste("E:/GIS/NatureServe/CODES/",LDspec[i],".shp",sep=""))
