...
z <- read.csv("G:/Boreal/InterannualVariability/BreedingLanduseDat20012014_drop.csv")
...
for (j in 1:length(LDspec)) {
	...
	write.csv(pred,file=paste("K:/Boreal/InterannualVariability/ecopred_2000-2013_",LDspec[j],"drop.csv",sep=""),row.names=FALSE)
	...
	write.csv(dat, file=paste("K:/Boreal/InterannualVariability/predclim20002013_",LDspec[j],"drop.csv",sep=""), row.names=FALSE)
}
